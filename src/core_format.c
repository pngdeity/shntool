...
static void spawn(child_args *process_args, FILE **readpipe, FILE **writepipe,
                  proc_info *pinfo, int child_type, FILE *inputstream)
 ...
 */
{
   ...
   * redirected standard handles".  URL:
  ...
  if (!CreatePipe(&hOutputReadTmp, &hOutputWrite, &sa, 0)) {
    st_error("error while creating output pipe for two-way communication with "
             "child process");
  }

  /* create the child stdin pipe */
  if (!CreatePipe(&hInputRead, &hInputWriteTmp, &sa, 0)) {
    st_error("error while creating input pipe for two-way communication with "
             "child process");
  }

  /* send child stderr to /dev/null */
  if (!(hErrorWrite = CreateFile(NULLDEVICE, GENERIC_WRITE, FILE_SHARE_WRITE,
  ...
  if (!DuplicateHandle(GetCurrentProcess(), hOutputReadTmp, GetCurrentProcess(),
    ...
    st_error("error while duplicating input pipe for two-way communication "
             "with child process");
  }
  ...
  if (!DuplicateHandle(GetCurrentProcess(), hInputWriteTmp, GetCurrentProcess(),
    ...
    st_error("error while duplicating error pipe for two-way communication "
             "with child process");
  }
  ...
  if (!CreateProcess(NULL, quoted_args, NULL, NULL, TRUE, 0, NULL, NULL, &si,
                     &pi)) {
    st_warning("error while launching helper program: [%s]",
               process_args->args[0]);
    *readpipe = NULL;
    *writepipe = NULL;
  ...
  if (!CloseHandle(hOutputWrite) || !CloseHandle(hInputRead) ||
  ...
  *writepipe = fdopen(_open_osfhandle((intptr_t)hInputWrite, 0), "wb");
  ...
  if ((pipe(pipe1) < 0) || (pipe(pipe2) < 0)) {
    perror("shntool: pipe");
    st_error("error while creating pipes for two-way communication with child "
             "process");
  }
  ...
  case 0:
    ...
    if (execvp(process_args->args[0], process_args->args) < 0) {
      ...
      st_error("error while launching helper program: [%s]",
               process_args->args[0]);
    }
  ...
  st_debug2("spawned %s process with pid %d and command line: %s",
            (CHILD_INPUT == child_type) ? "input" : "output", pinfo->pid,
            quoted_args);
}

static void arg_build(child_args *result, child_args *template,
                      char *filename) {
  int i;
  ...
  for (i = 0; i < template->num_args; i++) {
    if (NULL == template->args[i]) {
      if (0 == i) {
        st_error("while building argument list: encoder/decoder not defined or "
                 "unspecified");
      } else {
        st_error("while building argument list: encoder/decoder argument %d "
                 "not defined",
                 i);
      }
    }
...
static bool clobber_ask(char *filename) {
  ...
  if (!isatty(fileno(stdin)) || feof(stdin))
    st_error("standard input is not a terminal -- cannot prompt for overwrite "
             "action.  use \"-O always\" or \"-O never\".");
...
static void verify_format_generic(format_module *fm, child_args *template,
                                  char *type) {
  int i;
  bool found_filename;
  ...
  for (i = 0; i < template->num_args; i++) {
    if (NULL == template->args[i]) {
      if (0 == i) {
        st_error("%s argument list does not specify the %s for format: [%s]",
                 type, type, fm->name);
      } else {
        st_error("%s argument %d is undefined for format: [%s]", type, i,
                 fm->name);
      }
    }
  ...
  if (!found_filename)
    st_error("%s argument list does not contain filename placeholder [%s] for "
             "format: [%s]",
             type, FILENAME_PLACEHOLDER, fm->name);
}
...
FILE *launch_input(format_module *fm, char *filename, proc_info *pinfo) {
  ...
  if (fm->stdin_for_id3v2_kluge) {
    ...
    if (f && file_has_id3v2_tag) {
      st_debug1("input file has an ID3v2 tag -- attempting to trick helper "
                "program: [%s]",
                fm->decoder);
    } else {
    ...
    arg_build(&fm->input_args, &fm->input_args_template,
              (file_has_id3v2_tag) ? fm->stdin_for_id3v2_kluge : filename);
...
/* function to check if a file is about to be clobbered, and if so, asks whether
   this is OK */
{
  struct stat sz;
  bool retcode = FALSE;
