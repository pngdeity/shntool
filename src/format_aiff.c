...
static char default_decoder_args[] =
    "-t aiff " FILENAME_PLACEHOLDER " -t wav -";
...
format_module format_aiff = {"aiff",
...
static bool parse_aiff_header(char *filename, unsigned long *samples,
                              unsigned short *channels,
                              unsigned short *bits_per_sample)
/* generic function to parse an AIFF header and store certain values contained
   therein */
{
  FILE *f;
  ...
  *bits_per_sample = 0;
  ...
  if (tagcmp(tag, (unsigned char *)AIFF_FORM_TYPE_AIFF) &&
      tagcmp(tag, (unsigned char *)AIFF_FORM_TYPE_AIFC))
    goto invalid_aiff_header;
  ...
  if (is_compressed) {
    ...
    if (tagcmp(tag, (unsigned char *)AIFF_COMPRESSION_NONE) &&
        tagcmp(tag, (unsigned char *)AIFF_COMPRESSION_SOWT)) {
      st_debug1("found unsupported AIFF-C compression type [%c%c%c%c]", tag[0],
                tag[1], tag[2], tag[3]);
      goto invalid_aiff_header;
    }
  }
 ...
 */
{
  ...
  /* now set chunk size based on data size, and the canonical WAVE header size,
   * which sox generates */
