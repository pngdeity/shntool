...
enum { SPLIT_INPUT_UNKNOWN, SPLIT_INPUT_RAW, SPLIT_INPUT_CUE };
...
static void split_help() {
  ...
  st_info("\n");
  st_info("  -c num  start counting from num when naming output files (default "
          "is 1)\n");
  st_info("  -e len  prefix each track with len amount of lead-in from "
          "previous track (*)\n");
  ...
  st_info("  -m str  specify character manipulation string (alternating "
          "from/to)\n");
  st_info("  -n fmt  specify file count output format (default is %s -- ",
          SPLIT_NUM_FORMAT);
  for (i = 0; i < 3; i++) {
    st_info(SPLIT_NUM_FORMAT ", ", i + 1);
  }
  st_info("...)\n");
  st_info("  -t fmt  name output files in user-specified format based on CUE "
          "sheet fields.\n");
  ...
  st_info("\n");
  st_info("If no split point file is given, then split points are read from "
          "the terminal.\n");
  st_info("\n");
}
...
static void parse(int argc, char **argv, int *first_arg) {
  ...
  if (optind >= argc && !split_point_file)
    st_help("if file to be split is not given, then a split point file must be "
            "specified");
...
static void adjust_for_leadinout(wlong leadin_bytes, wlong leadout_bytes) {
  ...
  /* adjust WAVE data size for each file based on any lead-in/lead-outs
   * specified */
  for (pass = 0; pass < 2; pass++) {
    for (current = 0; current < numfiles; current++) {
      ...
      case 0:
        ...
        if (numfiles - 1 != current &&
            files[current + 1]->data_size < leadout_bytes)
...
static bool split_file(wave_info *info) {
  ...
  for (current = 0; current < numfiles; current++) {
    ...
    files[current]->chunk_size =
        files[current]->data_size + CANONICAL_HEADER_SIZE - 8;
    ...
    files[current]->exact_length =
        (double)files[current]->data_size / (double)info->rate;
    ...
    if (extract_track[current]) {
      ...
      if (NULL == (files[current]->output = open_output_stream(
                       outfilename, &files[current]->output_proc))) {
        prog_error(&proginfo);
      ...
      if (NULL == (files[current]->output = open_output(NULLDEVICE))) {
        prog_error(&proginfo);
        st_error("while skipping track %d: could not open output file: [%s]",
                 current + 1, NULLDEVICE);
      }
    ...
    if (write_n_bytes(files[current]->output, header, CANONICAL_HEADER_SIZE,
                      &proginfo) != CANONICAL_HEADER_SIZE) {
      prog_error(&proginfo);
      st_warning("error while writing %d-byte WAVE header",
                 CANONICAL_HEADER_SIZE);
      goto cleanup;
    }

    /* if this is not the first file, finish up writing previous file, and
     * simultaneously start writing to current file */
    if (0 != current) {
      /* write overlapping lead-in/lead-out data to both previous and current
       * files */
      if (transfer_n_bytes2(info->input, files[current]->output,
                            files[current - 1]->output,
                            leadin_bytes + leadout_bytes,
                            &proginfo) != leadin_bytes + leadout_bytes) {
        prog_error(&proginfo);
        st_warning("error while transferring %ld bytes of lead-in/lead-out",
                   leadin_bytes + leadout_bytes);
        goto cleanup;
      }

      /* pad and close previous file */
      if (PROB_ODD_SIZED_DATA(files[current - 1]) &&
          (1 != write_padding(files[current - 1]->output, 1, &proginfo))) {
        prog_error(&proginfo);
    ...
    if (transfer_n_bytes(info->input, files[current]->output, bytes_to_xfer,
                         &proginfo) != bytes_to_xfer) {
      prog_error(&proginfo);
    ...
    if (numfiles - 1 == current) {
      ...
      if (PROB_ODD_SIZED_DATA(files[current]) &&
          (1 != write_padding(files[current]->output, 1, &proginfo))) {
        prog_error(&proginfo);
        st_warning("error while NULL-padding odd-sized data chunk");
        goto cleanup;
      }
...
static void create_new_splitfile() {
  if (SPLIT_MAX_PIECES - 1 == numfiles)
    st_error("too many split files would be created -- maximum is %d",
             SPLIT_MAX_PIECES);
    ...
    if (files[whichfile]->data_size < CD_MIN_BURNABLE_SIZE) {
      ...
      if (extract_track[whichfile])
        st_warning("file %d will be too short to be burned%s", whichfile + 1,
                   (files[whichfile]->data_size % CD_BLOCK_SIZE)
                       ? ", and will not be cut on a sector boundary"
                       : "");
      ...
      files[whichfile]->problems |= PROBLEM_CD_BUT_BAD_BOUND;
      if (extract_track[whichfile])
        st_warning("file %d will not be cut on a sector boundary",
                   whichfile + 1);
    }
  } else {
...
static void cue_sprintf(int tracknum, char *filename) {
  ...
  if (extract_track[tracknum] && strstr(filename, psc)) {
    st_warning("converting illegal path character '%c' to '-' in "
               "CUE-originated filename: [%s]",
               PATHSEPCHAR, filename);
...
static void get_cue_field(unsigned char *field, unsigned char *line,
                          char *buf) {
...
static bool handle_cue_keyword(unsigned char *keyword, unsigned char *line) {
  ...
  if (!strcmp((char *)keyword, "PERFORMER")) {
    ...
    /* PERFORMER keywords in global section indicate artist name - will override
     * track artist names where not defined */
    if (cueinfo.in_global_section) {
  ...
  if ((!strcmp((char *)keyword, "INDEX")) &&
      (strstr((const char *)line, "INDEX 01")))
    return TRUE;
...
static void read_split_points_file(wave_info *info) {
  ...
  while (got_token || !feof(fd)) {
    ...
    if (0 == numfiles && 0 == current) {
      ...
      if (files[numfiles]->beginning_byte <= previous)
        st_error("split point %lu is not greater than previous split point %lu",
                 files[numfiles]->beginning_byte, previous);
  ...
  if (SPLIT_INPUT_CUE == input_type && cueinfo.format) {
    if (cueinfo.trackno < numfiles)
      st_error(
          "not enough TITLE keywords in CUE sheet to name each output file");
...
static void read_split_points_repeated(wave_info *info) {
  wlong repeated_split_size =
      smrt_parse((unsigned char *)repeated_split_point, info);
  wlong bytes_left;

  if (repeated_split_size >= info->data_size)
    st_error("repeated split size is not less than data size of input file -- "
             "nothing to do");
...
static void get_extractable_tracks() {
  ...
  if (extract_tracks) {
    ...
    while (p) {
      ...
      if (start < 1)
        st_error("track number to extract (%d) is not in range [1 .. %d]",
                 start, SPLIT_MAX_PIECES);
      if (end > SPLIT_MAX_PIECES)
        st_error("track number to extract (%d) is not in range [1 .. %d]", end,
                 SPLIT_MAX_PIECES);
      start--;
      end--;
...
static bool process_file(char *filename) {
  ...
  if (files[numfiles - 2]->beginning_byte == info->data_size) {
    ...
    files[numfiles - 1]->data_size =
        info->data_size - files[numfiles - 2]->beginning_byte;
