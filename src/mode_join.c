...
mode_module mode_join = {
    "join",   "shnjoin", "Joins PCM WAVE data from multiple files into one",
    CVSIDSTR, TRUE,      join_main,
    join_help};

#define JOIN_PREFIX "joined"

enum { JOIN_UNKNOWN, JOIN_NOPAD, JOIN_PREPAD, JOIN_POSTPAD };
...
static bool do_join() {
  ...
  joined_info->exact_length =
      (double)joined_info->data_size / (double)joined_info->rate;
  ...
  if (write_n_bytes(output, header, CANONICAL_HEADER_SIZE, &proginfo) !=
  ...
  for (i = 0; i < numfiles; i++) {
    ...
    if (transfer_n_bytes(files[i]->input, output, files[i]->data_size,
                         &proginfo) != files[i]->data_size) {
      prog_error(&proginfo);
      st_warning("error while transferring %lu bytes of data",
                 files[i]->data_size);
      goto cleanup;
    }
  ...
  if ((JOIN_NOPAD == pad_type) && PROB_ODD_SIZED_DATA(joined_info) &&
      (1 != write_padding(output, 1, NULL))) {
    prog_error(&proginfo);
