...
static char default_decoder_args[] =
    "--decode " FILENAME_PLACEHOLDER " --output -";
static char default_encoder_args[] =
    "--encode - --output " FILENAME_PLACEHOLDER;

static bool is_our_file(char *);

format_module format_ofr = {"ofr",
...
static bool is_our_file(char *filename) {
  return (check_for_magic(filename, OPTIMFROG_MAGIC, 0) ||
          check_for_magic(filename, OPTIMFROG_MAGIC_OLD, 0));
}
