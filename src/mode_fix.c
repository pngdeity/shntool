...
static void fix_help() {
  ...
  st_info("\n");
  st_info("  -b      shift track breaks backward to previous sector boundary "
          "(default)\n");
...
static void calculate_breaks_backward() {
  ...
  files[numfiles - 1]->new_data_size =
      files[numfiles - 1]->data_size + remainder;
...
static void calculate_breaks_round() {
  ...
  files[numfiles - 1]->new_data_size =
      files[numfiles - 1]->data_size + give_or_take;
  files[numfiles - 1]->beginning_byte = begin_before;
  files[numfiles - 1]->new_beginning_byte = begin_after;
}
...
static void calculation_sanity_check() {
  ...
  if (old_total != new_total) {
    ...
    st_info(
        "Shift type: %s\n",
        (SHIFT_BACKWARD == desired_shift)
            ? "backward"
            : ((SHIFT_FORWARD == desired_shift)
                   ? "forward"
    ...
    for (i = 0; i < numfiles; i++)
      st_info("file %2d:  data size = %10lu, new data size = %10lu\n", i + 1,
              files[i]->data_size, files[i]->new_data_size);
...
static bool reopen_input_file(int i, progress_info *proginfo) {
  ...
  if (NULL ==
      (header = malloc(files[i]->header_size * sizeof(unsigned char)))) {
    prog_error(proginfo);
    st_warning("could not allocate %d-byte WAVE header", files[i]->header_size);
    return FALSE;
  }

  if (read_n_bytes(files[i]->input, header, files[i]->header_size, NULL) !=
      files[i]->header_size) {
    prog_error(proginfo);
    st_warning("error while reading %d-byte WAVE header",
               files[i]->header_size);
    st_free(header);
    return FALSE;
  }
...
static bool open_this_file(int i, char *outfilename, progress_info *proginfo) {
  ...
  create_output_filename(files[i]->filename, files[i]->input_format->extension,
                         outfilename);
  ...
  if (NULL == (files[i]->output =
                   open_output_stream(outfilename, &files[i]->output_proc))) {
    prog_error(proginfo);
  ...
  if ((numfiles - 1 == i) && pad)
    put_data_size(header, CANONICAL_HEADER_SIZE,
                  files[i]->new_data_size + pad_bytes);
  else {
    put_data_size(header, CANONICAL_HEADER_SIZE, files[i]->new_data_size);
    if (files[i]->new_data_size & 1)
      put_chunk_size(header,
                     (files[i]->new_data_size + 1) + CANONICAL_HEADER_SIZE - 8);
  }

  if (write_n_bytes(files[i]->output, header, CANONICAL_HEADER_SIZE,
...
static bool write_fixed_files() {
  ...
  while (cur_input < numfiles && cur_output < numfiles) {
    ...
    if (transfer_n_bytes(files[cur_input]->input, files[cur_output]->output,
                         bytes_to_xfer, &proginfo) != bytes_to_xfer) {
      prog_error(&proginfo);
    ...
    if (0 == bytes_needed) {
      ...
      if (numfiles - 1 == cur_output) {
        if (pad) {
          ...
          if ((files[cur_output]->new_data_size & 1) &&
              (1 != write_padding(files[cur_output]->output, 1, NULL))) {
            prog_error(&proginfo);
...
static bool process(int argc, char **argv, int start) {
  ...
  if (skip) {
    if (i != 0) {
      st_warning("skipping first %d file%s because %s would not be changed", i,
                 (1 == i) ? "" : "s", (1 == i) ? "it" : "they");
      for (j = 0; j < i; j++)
        st_free(files[j]);
