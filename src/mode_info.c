...
mode_module mode_info = {
    "info",   "shninfo", "Displays detailed information about PCM WAVE data",
    CVSIDSTR, FALSE,     info_main,
    info_help};
...
static bool show_info(wave_info *info) {
  ...
  st_output("WAVE format:                  0x%04x (%s)\n", info->wave_format,
            format_to_str(info->wave_format));
  ...
  st_output("  Cut on sector boundary:     %s\n",
            (PROB_NOT_CD(info)) ? "n/a"
                                : ((PROB_BAD_BOUND(info)) ? "no" : "yes"));

  st_output("  Sector misalignment:        ");
  if (PROB_NOT_CD(info))
    st_output("n/a\n");
  else
    st_output("%lu byte%s\n", info->data_size % CD_BLOCK_SIZE,
              (1 == (info->data_size % CD_BLOCK_SIZE)) ? "" : "s");
  ...
  if (!info->input_format->is_compressed &&
    ...
    if (PROB_TRUNCATED(info)) {
      missing_bytes =
          info->total_size - (info->actual_size - info->id3v2_tag_size);
