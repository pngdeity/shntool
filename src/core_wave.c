...
/* determines whether the given filename (info->filename) is a regular file, and
   is readable */
{
  struct stat sz;
  FILE *f;
  ...
  if (stat(info->filename, &sz)) {
    if (errno == ENOENT)
      ...
      st_warning("encountered bad address while opening file: [%s]",
                 info->filename);
    else if (errno == ENOMEM)
      st_warning("kernel ran out of memory while opening file: [%s]",
                 info->filename);
    else if (errno == ENAMETOOLONG)
      st_warning("filename too long to open file: [%s]", info->filename);
    else
      st_warning("encountered system error [%s] while opening file: [%s]",
  ...
  if (NULL == (f = fopen(info->filename, "rb"))) {
    st_warning("encountered error [%s] while opening file: [%s]",
               strerror(errno), info->filename);
    return FALSE;
  }
...
bool do_header_kluges(unsigned char *header, wave_info *info) {
  ...
  if (info->input_format && info->input_format->input_header_kluge) {
    st_debug1("performing [%s] input header kluge on file: [%s]",
              info->input_format->name, info->filename);
    retval = info->input_format->input_header_kluge(header, info);
  }

  if (!retval)
    st_warning("[%s] input header kluge failed while processing file: [%s]",
               info->input_format->name, info->filename);
...
/* verifies that data coming in on the file descriptor info->input describes a
   valid WAVE header */
{
  ...
  if (!read_tag(info->input, tag) || tagcmp(tag, (unsigned char *)WAVE_RIFF)) {
    if (verbose) {
      if (!tagcmp(tag, (unsigned char *)AIFF_FORM)) {
        st_warning(
            "encountered unsupported AIFF data while processing file: [%s]",
            info->filename);
      } else {
        st_warning(
            "WAVE header is missing RIFF tag while processing file: [%s]",
            info->filename);
      }
    }
  ...
  if (!read_le_long(info->input, &info->chunk_size)) {
    st_warning("could not read chunk size from WAVE header while processing "
               "file: [%s]",
               info->filename);
    return FALSE;
  }
  ...
  if (!read_tag(info->input, tag) || tagcmp(tag, (unsigned char *)WAVE_WAVE)) {
    st_warning("WAVE header is missing WAVE tag while processing file: [%s]",
               info->filename);
    return FALSE;
  }
  ...
  for (;;) {
    if (!read_tag(info->input, tag) || !read_le_long(info->input, &le_long)) {
      st_warning("reached end of file while looking for fmt tag while "
                 "processing file: [%s]",
                 info->filename);
      return FALSE;
    }

    st_debug1("found chunk: [%c%c%c%c] with length: %lu", tag[0], tag[1],
              tag[2], tag[3], le_long);
  ...
  if (le_long < 16) {
    st_warning(
        "fmt chunk in WAVE header was too short while processing file: [%s]",
        info->filename);
    return FALSE;
  }
  ...
  if (!read_le_short(info->input, &info->wave_format)) {
    st_warning(
        "reached end of file while reading format while processing file: [%s]",
        info->filename);
    return FALSE;
  }
  ...
  if (!read_le_short(info->input, &info->channels)) {
    st_warning(
        "reached end of file reading channels while processing file: [%s]",
        info->filename);
    return FALSE;
  }
  ...
  if (!read_le_long(info->input, &info->samples_per_sec)) {
    st_warning(
        "reached end of file reading samples/sec while processing file: [%s]",
        info->filename);
    return FALSE;
  }

  if (!read_le_long(info->input, &info->avg_bytes_per_sec)) {
    st_warning("reached end of file reading average bytes/sec while processing "
               "file: [%s]",
               info->filename);
    return FALSE;
  }
  ...
  if (!read_le_short(info->input, &info->block_align)) {
    st_warning(
        "reached end of file reading block align while processing file: [%s]",
        info->filename);
    return FALSE;
  }
  ...
  if (!read_le_short(info->input, &info->bits_per_sample)) {
    st_warning(
        "reached end of file reading bits/sample while processing file: [%s]",
        info->filename);
    return FALSE;
  }
  ...
  if (le_long) {
    ...
    while (bytes > 0) {
      if (fread(buf, 1, 1, info->input) != 1) {
        st_warning("reached end of file jumping ahead %lu bytes while "
                   "processing file: [%s]",
  ...
  for (;;) {
    if (!read_tag(info->input, tag) || !read_le_long(info->input, &le_long)) {
      st_warning("reached end of file looking for data tag while processing "
                 "file: [%s]",
                 info->filename);
      return FALSE;
    }
    ...
    while (bytes > 0) {
      if (fread(buf, 1, 1, info->input) != 1) {
        st_warning("reached end of file jumping ahead %lu bytes when looking "
                   "for data tag while processing file: [%s]",
                   le_long, info->filename);
        return FALSE;
      }
      bytes--;
  ...
  info->rate = ((wint)info->samples_per_sec * (wint)info->channels *
                (wint)info->bits_per_sample) /
               8;

  info->total_size = info->chunk_size + 8;

  /* per RIFF specs, data chunk is always an even number of bytes - a NULL pad
   * byte should be present when data size is odd */
  ...
  if (info->channels == CD_CHANNELS &&
      ...
      */
      info->avg_bytes_per_sec == CD_RATE && info->rate == CD_RATE) {
...
/* if filename is NULL, return a fresh wave_info struct with all data zero'd
 * out. Otherwise, check that the file referenced by filename exists, is
 * readable, and contains a valid WAVE header - if so, return a wave_info struct
 * filled out with the values in the WAVE header, otherwise return NULL.
 */
{
  int i, bytes;
  FILE *f;
  ...
  for (i = 0; st_formats[i]; i++) {
    ...
    if (st_formats[i]->is_our_file) {
      ...
      /* otherwise, check for format-defined magic string at a predefined offset
       * (if defined) */
      if (!check_for_magic(info->filename, st_formats[i]->magic,
                           st_formats[i]->magic_offset))
        continue;
    }
    ...
    if (NULL ==
        (f = open_input_internal(info->filename, &info->file_has_id3v2_tag,
                                 &info->id3v2_tag_size))) {
      st_warning("open failed while setting ID3v2 flag for file: [%s]",
                 info->filename);
      goto invalid_wave_data;
    }
    ...
    /* make sure the file can be opened by the output format - this skips over
     * any ID3v2 tags in the stream */
    if (!open_input_stream(info)) {
      st_debug1(
          "input file could not be opened for streaming input by format: [%s]",
          st_formats[i]->name);
      goto invalid_wave_data;
    }

    /* make sure we can read data from the output format (primarily to ensure
     * the decoder is sending us data) */
    if (1 != fread(&buf, 1, 1, info->input)) {
      st_snprintf(msg, BUF_SIZE,
  ...
  st_warning("none of the builtin format modules handle input file: [%s]",
             info->filename);

  if ((f = open_input_internal(info->filename, &info->file_has_id3v2_tag,
                               &info->id3v2_tag_size))) {
...
void put_data_size(unsigned char *header, int header_size,
                   unsigned long new_data_size)
/* replaces the size reported in the "data" chunk of the wave header with the
   new size - also updates chunk size at beginning of the wave header */
{
  if (NULL == header)
    return;
