...
/* md5.c - Functions to compute MD5 message digest of files or memory blocks
   ...
   Inc. NOTE: The canonical source of this file is maintained with the GNU C
...
void *md5_finish_ctx(struct md5_ctx *ctx, void *resbuf) {
  ...
  *(md5_uint32 *)&ctx->buffer[bytes + pad + 4] =
      SWAP_MD5((ctx->total[1] << 3) | (ctx->total[0] >> 29));
...
int md5_stream(FILE *stream, void *resblock) {
  ...
  */
  ...
  while (1) {
    ...
    while (1) {
      n = read_n_bytes(stream, (unsigned char *)(global_buffer + sum),
                       BLOCKSIZE - sum, &proginfo);

      sum += n;
      totalbytes += n; /* shntool */

      /* shntool: if total bytes read exceeded the WAVE data size, discard the
       * rest and adjust sum */
...
void *sha1_finish_ctx(struct sha1_ctx *ctx, void *resbuf) {
  ...
  *(md5_uint32 *)&ctx->buffer[bytes + pad] =
      SWAP_SHA1((ctx->total[1] << 3) | (ctx->total[0] >> 29));
...
static bool generate_audio_hash_single(wave_info *info) {
  ...
  if (read_n_bytes(info->input, header, info->header_size, NULL) !=
      info->header_size) {
    prog_error(&proginfo);
    st_warning("error while discarding %d-byte WAVE header from file: [%s]",
...
static bool generate_audio_hash_composite(wave_info *info) {
  ...
  if (read_n_bytes(info->input, header, info->header_size, NULL) !=
      info->header_size)
    st_error("error while discarding %d-byte WAVE header from file: [%s]",
             info->header_size, info->filename);
