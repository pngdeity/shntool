...
#define PROB_HDR_NOT_CANONICAL(f)                                              \
  ((f->problems) & (PROBLEM_HEADER_NOT_CANONICAL))
...
typedef struct _wave_info {
  ...
  unsigned long
      problems; /* bitmap of problems found with the file, see above   */
  ...
  bool stream_has_id3v2_tag; /* does the decoded input stream contain an ID3v2
                                tag? */
  wlong id3v2_tag_size; /* size of the ID3v2 tag this file contains, if any */
} wave_info;

/* returns a wave_info struct, filled out with the values of the WAVE data
 * contained in the filename given. */
/* If called with NULL as the argument, then a wave_info struct is returned with
 * all fields zero'd out.     */
...
/* kluges the WAVE header to get correct values when helper programs don't
 * provide them */
bool do_header_kluges(unsigned char *, wave_info *);

/* verifies that data coming in on the associated file descriptor describes a
 * valid WAVE header */
