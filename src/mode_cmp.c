...
mode_module mode_cmp = {
    "cmp",    "shncmp", "Compares PCM WAVE data in two files", CVSIDSTR, FALSE,
    cmp_main, cmp_help};

#define CMP_MATCH_SIZE                                                         \
  2352 /* arbitrary number of bytes that must match before attempting          \
...
static void cmp_help() {
  ...
  st_info("  -s      check if WAVE data in the files is identical modulo a "
          "byte-shift\n");
  st_info("\n");
}
...
static void check_headers(wave_info *info1, wave_info *info2, int shift) {
  ...
  if (data_size1 != data_size2)
    st_warning("WAVE data size differs between these files -- will check up to "
               "smaller size");
}

static int memfuzzycmp(unsigned char *str1, unsigned char *str2, int len,
                       int fuzz) {
...
static bool cmp_files(wave_info *info1, wave_info *info2, int shift) {
  ...
  wlong bytes_to_check, bytes_checked = 0, bytes,
                        shifted_data_size1 = info1->data_size,
                        shifted_data_size2 = info2->data_size, xfer_size;
  ...
  while (bytes_to_check > 0) {
    ...
    if (-1 != (offset = memfuzzycmp(buf1, buf2, bytes, 0))) {
      ...
      if (!list) {
        prog_error(&proginfo);
        st_error("WAVE data differs at byte offset: %lu",
  ...
  if (!differed) {
    ...
    st_info("\n");
    st_info("%s of these files are identical",
            (0 == shift) ? "Contents" : "Aligned contents");
    if (shifted_data_size1 != shifted_data_size2)
      st_info(" (up to the first %lu bytes of WAVE data)",
              min(shifted_data_size1, shifted_data_size2));
    st_info(".\n");
  } else {
    success = FALSE;
    st_info("\n");
...
static void open_and_read_beginning(wave_info *info, unsigned char *buf,
                                    int bytes) {
  open_file(info);
...
static bool shift_comparison(wave_info *info1, wave_info *info2) {
  ...
  if (!found_possible_shift) {
    prog_error(&proginfo);
    st_error(
        "these files do not share identical data within the first %ld bytes.",
        cmp_size);
  }
  ...
  if (0 == shift) {
    ...
    st_info("%d extra bytes (%d extra samples", real_shift,
            (real_shift + 2) / 4);
    if (!PROB_NOT_CD(info1) && !PROB_NOT_CD(info2))
      st_info(", or %d extra sectors",
              (real_shift + (CD_BLOCK_SIZE / 2)) / CD_BLOCK_SIZE);
    st_info(")\n");
    st_info("\n");
...
static bool process(int argc, char **argv, int start) {
  ...
  if (!(filename1 = input_get_filename()) ||
      !(filename2 = input_get_filename())) {
