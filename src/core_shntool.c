...
static void show_format_module_args() {
  ...
  for (i = 0; st_formats[i]; i++) {
    ...
    if (st_formats[i]->decoder) {
      ...
      st_info(" -i \"%s %s %s\"", st_formats[i]->name, st_formats[i]->decoder,
              st_formats[i]->decoder_args);
      st_info("\n");
      showed_format = 1;
    }
    ...
    if (st_formats[i]->encoder) {
      ...
      st_info(" -o \"%s ext=%s %s %s\"", st_formats[i]->name,
              st_formats[i]->extension, st_formats[i]->encoder,
              st_formats[i]->encoder_args);
      st_info("\n");
      showed_format = 1;
    }
  }
  ...
  if (st_modes[0]) {
    for (i = 0; st_modes[i]; i++) {
      ...
      if (NULL == st_modes[i]->run_main)
        st_error("mode module does not provide run_main(): [%s]",
                 st_modes[i]->name);
    }
  ...
  if (st_formats[0]) {
    for (i = 0; st_formats[i]; i++) {
      ...
      if (!st_formats[i]->supports_input && !st_formats[i]->supports_output)
        st_error("format module doesn't support input or output: [%s]",
                 st_formats[i]->name);

      if (!st_formats[i]->supports_input && st_formats[i]->decoder)
        st_error(
            "format module doesn't support input but defines a decoder: [%s]",
            st_formats[i]->name);

      if (!st_formats[i]->supports_output && st_formats[i]->encoder)
        st_error(
            "format module doesn't support output but defines an encoder: [%s]",
            st_formats[i]->name);
    }
    ...
    for (i = 0; st_formats[i]; i++) {
      for (j = i + 1; st_formats[j]; j++) {
        if (!strcmp(st_formats[i]->name, st_formats[j]->name))
          st_error("found formats with identical name: [%s]",
                   st_formats[i]->name);
      }
    }
  } else {
...
static bool parse_main(int argc, char **argv) {
  ...
  for (i = 0; st_modes[i]; i++) {
    if (NULL != st_modes[i]->alias &&
        !strcmp(st_priv.progname, st_modes[i]->alias)) {
